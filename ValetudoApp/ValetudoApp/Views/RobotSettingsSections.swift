...
struct AutoEmptyDockSettingsView: View {
    ...
    private func loadPresets() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to load auto empty dock presets: \(error, privacy: .public)")
        }
    }
    ...
    private func selectPreset(_ preset: String) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set auto empty dock preset: \(error, privacy: .public)")
        }
    }
}
...
struct QuirksView: View {
    ...
    private func loadQuirks() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to load quirks: \(error, privacy: .public)")
        }
    }
    ...
    private func setQuirk(id: String, value: String) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set quirk: \(error, privacy: .public)")
        }
    }
}
...
struct WifiSettingsView: View {
    ...
    private func loadStatus() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to load WiFi status: \(error, privacy: .public)")
        }
    }
    ...
    private func scanNetworks() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to scan WiFi: \(error, privacy: .public)")
        }
    }
    ...
    private func connectToNetwork(_ network: WifiNetwork) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to connect to WiFi: \(error, privacy: .public)")
        }
    }
}
...
struct MQTTSettingsView: View {
    ...
    private func loadConfig() async {
        guard let api = api else {
            sectionsLogger.error("MQTT: No API available")
            return
        }
        isLoading = true
        defer { isLoading = false }

        sectionsLogger.debug("Loading MQTT config...")
        do {
            config = try await api.getMQTTConfig()
            sectionsLogger.debug("MQTT config loaded: enabled=\(config?.enabled ?? false, privacy: .public)")
            if let config = config {
                ...
                sectionsLogger.debug("MQTT username from API: '\(loadedUsername == "<redacted>" ? "<redacted>" : "set", privacy: .public)'")
            ...
            sectionsLogger.error("Failed to load MQTT config: \(error, privacy: .public)")
        }
    }
    ...
    private func saveConfig() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to save MQTT config: \(error, privacy: .public)")
        }
    }
}
...
struct NTPSettingsView: View {
    ...
    private func loadConfig() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to load NTP config: \(error, privacy: .public)")
        }
    }

    private func loadStatus() async {
        guard let api = api else {
            sectionsLogger.error("NTP: No API available")
            return
        }
        sectionsLogger.debug("Loading NTP status...")
        do {
            status = try await api.getNTPStatus()
            sectionsLogger.debug("NTP status loaded successfully")
        } catch {
            sectionsLogger.error("Failed to load NTP status: \(error, privacy: .public)")
        }
    }
    ...
    private func saveConfig() async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to save NTP config: \(error, privacy: .public)")
        }
    }
...
struct StationSettingsView: View {
    ...
    private func loadSettings() async {
        ...
        if hasMopDockDryingTime {
            do {
                ...
                sectionsLogger.debug("Mop dock drying time not supported: \(error, privacy: .public)")
            }
        }
        ...
        if hasAutoEmptyDockDuration {
            do {
                ...
                sectionsLogger.debug("Auto empty dock duration not supported: \(error, privacy: .public)")
            }
        }
    ...
    private func setMopDockAutoDrying(_ enabled: Bool) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set mop dock auto drying: \(error, privacy: .public)")
    ...
    private func setWashTemperature(_ preset: String) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set wash temperature: \(error, privacy: .public)")
        }
    }
    ...
    private func setDryingTime(_ preset: String) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set mop dock drying time: \(error, privacy: .public)")
        }
    }
    ...
    private func setAutoEmptyDockDuration(_ preset: String) async {
        ...
        do {
            ...
            sectionsLogger.error("Failed to set auto empty dock duration: \(error, privacy: .public)")
        }
    }
